...
from sample.sql_alchemy.src.modules.user_data_validator import is_valid_cpf, is_valid_email
...
def create_app(config=None):
    ...
    app.config["SQLALCHEMY_DATABASE_URI"] = "mysql+pymysql://root:admin123@127.0.0.1:3306/flask-alchemy"
    ...
    def login():
        try:
            ...
            if user and bcrypt.checkpw(str.encode(password), user.password.encode('utf-8')):
                login_user(user)
    ...
    def update_user(user_id):
        try:
            ...
            if user_id != current_user.id and current_user.role != UserRoles.ADMIN.value:
                return jsonify({"message": "Operação não permitida!"}), 403
